...
char model_name[30] = "YOLO v2";


static VideoCapture cap;
static float fps = 0;
static float demo_thresh = 0.4;
static float demo_thresh_low = 0.4;
...
void *detect_roi_in_thread(void *ptr){
	...
	/* for(unsigned int i = 0; i < bbox_draw.size(); i++){ */
		/* rectangle(cur_frame, bbox_draw[i], color, 5, 1); */
	/* } */
...
#ifdef DRAW_RECTANGLE
		//draw the mapping detections
		
		for(unsigned int j = 2; j < 5; j++){
			for(unsigned int i = 0; i < cam[j].size(); i++){
				Rect2d temp;
				temp.x = cam[j][i].lx + roi_left_mid_x;
				temp.y = cam[j][i].ly + roi_left_mid_y;
				temp.width = cam[j][i].rx-cam[j][i].lx;
				temp.height = cam[j][i].ry-cam[j][i].ly;
				
				//Fusion
				rectangle(cur_frame, temp, Scalar(0,0,100), 5, 1);
				if(j == 2 || j == 3)	rectangle(cur_frame, temp, Scalar(255,0,127), 5, 1);
				if(j == 4 || j == 5)	rectangle(cur_frame, temp, Scalar(255,0,127), 5, 1);
			}
		}
#endif
		...
		if(class1 == 2){
			...
			if(prob > demo_thresh_low){
				...
				if(frame_offset == LEFT_MID){
					...
					//left right
					//left to left
					left += roi_left_right_x;
					right += roi_left_right_x;
					top += roi_left_right_y;
					bot += roi_left_right_y;
					...
					//up right
					left += roi_up_right_x;
					right += roi_up_right_x;
					top += roi_up_right_y;
					bot += roi_up_right_y;

					/*
					if (right < roi_up_right_x || left > roi_up_right_x + NET_DIM){
						pass_flag = 1;
						continue;
					}
					*/
					...
					//right mid
					left += roi_right_mid_x;
					right += roi_right_mid_x;
					top += roi_right_mid_y;
					bot += roi_right_mid_y;
					...
					//right right
					left += roi_right_right_x;
					right += roi_right_right_x;
					top += roi_right_right_y;
					bot += roi_right_right_y;
					
					if (right < roi_right_right_x || left > roi_right_right_x + NET_DIM){
						pass_flag = 1;
						continue;
					}
				...
				if (right < roi_left_mid_x || left > roi_left_mid_x + 2 * NET_DIM || pass_flag == 1){}
				else{
					...
					//shift roi_left_x, roi_left_y to 0
					left -= roi_left_mid_x;
					right -= roi_left_mid_x;
					top -= roi_left_mid_y;
					bot -= roi_left_mid_y;
					...
					temp.ry = bot;
					temp.prob = prob;
					temp.hilbert_value = get_hilbert_value((left-roi_left_mid_x+right-roi_left_mid_x)/2, (top-roi_left_mid_y+bot-roi_left_mid_y)/2);
					temp.level = 0;
					temp.camera_id = frame_offset;
					cam[frame_offset].push_back(temp);
					cam_backup[frame_offset].push_back(temp);
				}
			}
        }
    }
